...
import {useSessionContextState} from '../../hooks';

describe('useSessionContextState', () => {
  it('returns default user profile on init', () => {
  ...
  it('memoizes the returned object when state is unchanged (no-op update)', () => {
    const {result} = renderHook(() => useSessionContextState());
  ...
  it('returns a new memoized object when userProfile changes', () => {
    const {result} = renderHook(() => useSessionContextState());
    const firstRef = result.current;
    const firstSetter = result.current.setUserProfile;
    ...
    expect(result.current.setUserProfile).toBe(firstSetter);
  });
});
